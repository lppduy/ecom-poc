...
| Kafka UI | Local topic inspector — `http://localhost:8088` |
...
```
BEGIN TX
  SELECT * FROM stocks WHERE product_id = ? FOR UPDATE  ← lock row
...
`POST /orders` requires `Idempotency-Key` header. If a key is seen again, the existing order is returned — safe to retry on timeout/network error.
